...
-- Drop enums if they exist (after tables)
...
-- Make created_at and updated_at differ per row (apply same concept as users)
INSERT INTO system_role (name, description, code, created_at, updated_at) VALUES
('Admin', 'System administrator', 'admin', NOW() - INTERVAL '1 minute', NOW()),
('Instructor', 'Main instructor', 'instructor', NOW() - INTERVAL '1 day', NOW() - INTERVAL '1 day' + INTERVAL '1 minute'),
('Learner', 'Student learner', 'learner', NOW() - INTERVAL '2 days', NOW() - INTERVAL '2 days' + INTERVAL '1 minute'),
('Replacement Instructor', 'Backup instructor', 'replacement_instructor', NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days' + INTERVAL '1 minute'),
('Visiting Instructor', 'Guest instructor', 'visiting_instructor', NOW() - INTERVAL '4 days', NOW() - INTERVAL '4 days' + INTERVAL '1 minute');
...
-- Admin user (ensure different timestamps within row)
INSERT INTO public."user" (name, email, role_id, status, created_at, updated_at)
VALUES ('Admin User', 'admin@example.com', 1, 'active', NOW() - INTERVAL '5 minutes', NOW() - INTERVAL '4 minutes');
...
-- Visiting Instructors (role_id = 5) - make timestamps differ
INSERT INTO public."user" (name, email, role_id, status, created_at, updated_at)
VALUES 
  ('Visiting Instructor 001', 'visiting001@example.com', 5, 'active', NOW() - INTERVAL '10 minutes', NOW() - INTERVAL '9 minutes'),
  ('Visiting Instructor 002', 'visiting002@example.com', 5, 'active', NOW() - INTERVAL '9 minutes', NOW() - INTERVAL '8 minutes');

-- Replacement Instructors (role_id = 4) - make timestamps differ
INSERT INTO public."user" (name, email, role_id, status, created_at, updated_at)
VALUES 
  ('Replacement Instructor 001', 'replacement001@example.com', 4, 'active', NOW() - INTERVAL '8 minutes', NOW() - INTERVAL '7 minutes'),
  ('Replacement Instructor 002', 'replacement002@example.com', 4, 'active', NOW() - INTERVAL '7 minutes', NOW() - INTERVAL '6 minutes');
...
-- Learners Cohort 2 (fix column name to subgroup_id)
INSERT INTO public."user" (name, email, role_id, status, cohort_id, subgroup_id, created_at, updated_at)
SELECT 
