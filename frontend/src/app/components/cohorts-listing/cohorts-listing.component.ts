...
export class CohortsListingComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    const url = `http://localhost:7860/api/cohorts?page=${this.page}&size=${this.size}`;
    fetch(url)
      .then(r => r.json())
      .then((data: any) => {
        let items: any[] = [];
        ...
        this.rows = items.map((raw: any) => {
          ...
          if (Array.isArray(rawSubs)) {
            subgroups = rawSubs.map((sg: any) => typeof sg === 'string' ? sg : (sg?.name ?? String(sg)));
  ...
  changeSize(val: string | number) {
    ...
    this.size = n; this.page = 1; this.load();
  }
